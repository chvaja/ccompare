...
class CCompare {
  ...
  async search (query) {
    ...
    const oracleRef = sym.ref ? [ sym.ref[0], sym.ref[2], sym.ref[1] ] : [ 'coinbase-pro', 'usd', query.target ]
    const oracleRate = this.oracleRate(...oracleRef)
    const items = await Promise.all(this.providers.map(async p => {
      const obj = { seller: p.id, price: null }
      if (!p.resolve) {
        return obj
      }
      //console.log(`-> ${p.id} started`)
      const res = await p.resolve(query)
    ...
    out.push({
      ...
      price: String((Number(await oracleRate) * Number(query.value))/ Number(this.rates[`${query.source.toUpperCase()}_USD`])),
    ...
    const altRate = this.rates[query.source.toUpperCase() + '_USD']
